...
export function resultsKeyboard(
  ...
  if (totalPages > 1) {
    const nav: any[] = [];
    if (page > 0) nav.push({ text: "« הקודם", callback_data: `pg_${queryId}_${page}_${page - 1}` });
    nav.push({ text: `${page + 1}/${totalPages}`, callback_data: "noop" });
    if (page < totalPages - 1) nav.push({ text: "הבא »", callback_data: `pg_${queryId}_${page}_${page + 1}` });
    rows.push(nav);
  }
  rows.push([
